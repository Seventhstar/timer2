...
(function() {
  ...
  this.disable_input = function(cancel) {
    var $cell, $cells, item_id;
    if (cancel == null) {
      cancel = true;
    }
  ...
  $(document).ready(function() {
    $('.container').on('click', 'span.check_img', function() {
      var checked, chk, item_id, model, tr, upd, upd_p;
      ...
      upd_p[chk] = checked;
      upd['upd'] = upd_p;
      return upd_param(decodeURIComponent($.param(upd)) + '&model=' + model + '&id=' + item_id);
    });
    $('.container').on('click', 'span.edit', function() {
      var $cell, $cells, $row, item_id, table;
      item_id = $(this).attr('item_id');
      $row = $(this).parents('');
      disable_input();
    ...
    $('.container').on('click', 'span.icon_cancel', function() {
      disable_input();
    });
    $('#goal_name').focus();
    $('.container').on('click', 'span.icon_apply', function() {
      var inputs, item_id, model;
      model = $(this).closest('table').attr('model');
      item_id = $(this).attr('item_id');
      inputs = $('input[name^=upd]');
      upd_param(inputs.serialize() + '&model=' + model + '&id=' + item_id);
    });
    $('body').on('keyup keypress', '#goal_name', function(e) {
      var code;
...
;
(function() {
  this.goal_modify = function(method, id) {
    $.ajax({
      url: '/ajax/' + method,
      data: {
        'id': id
      ...
      success: function() {
        ...
        return show_ajax_message('успешно', 'notice');
      }
    });
  };

  this.toolbar_pressed = function(e, el) {
    var del, del_url, item_id, location, url;
  ...
  $(document).ready(function() {
    ...
    return $('.btn-toolbar').on('toolbarItemClick', function(e, el) {
...
(function( $ ){
    ...
    function defaultFormatMilliseconds(millis) {
        ...
        x = millis / 1000;
        millisec = Math.floor(millis/100 % 10);
        ...
        // x /= 24;
        // days = Math.floor(x);
        return [[pad2(hours), pad2(minutes), pad2(seconds)].join(':'),millisec].join('.');
    }
    ...
    var methods = {
        ...
        render: function() {
            var $this = $(this),
                data = $this.data('stopwatch');
            $("#demo2 span").html(data.formatter((new Date()-data.stTime), data));
            $this.attr("last_time", Math.round((new Date()-data.stTime )/100)/10);
        },
    ...
    // Define the function
    $.fn.stopwatch = function( method, displayblock ) {
        if (methods[method]) {
...
(function() {
  $(document).ready(function() {
    return $(document).on('click', 'span.delete', function() {
      var del, del_url, item_id, url;
      url = '/' + $('form').attr('action').split('/')[1];
      item_id = $(this).attr('item_id');
      del_url = url + '/' + item_id;
      del = confirm('Действительно удалить?');
...
(function() {
  $(document).ready(function() {
    $('#table_tasks').on('click', '.task.btn', function() {
      $('.task.btn').removeClass('active');
      $(this).addClass('active');
    });
    $('#table_type').on('click', '.btn', function() {
      $('.btn').removeClass('active');
      $(this).addClass('active');
    });
    $(document).on('click', '.del', function() {
      var del, del_url, item_id, op;
      item_id = $(this).attr('el_id');
      del_url = '/used_times/' + item_id;
      op = $('.opened').map(function() {
        return $(this).attr('id').substr(5);
      }).get().join();
      del = confirm('Действительно удалить?');
      if (!del) {
        return;
